...
export type DecryptableMessage = RequireAtLeastOne<{
  clientUrl ?: string
}, 'clientUrl'> & RequiredDecryptionMessage
...
//@ts-ignore
export const decryptMedia :  (file: DecryptableMessage | boolean, useragentOverride?: string) => Promise<Buffer> = async (message: DecryptableMessage | Message | boolean, useragentOverride?: string) => {
  ...
  if (!message || !message.mediaKey || !message.filehash || !message.mimetype || !message.type || !message.size) {
    if (missingProps.length == 1 && missingProps[0]==="size") {
      if(!nonSizeTypes.includes(message.type)) console.warn("size property is missing. File will fail an integrity check.")
    } 
  ...
  try {
    while (haventGottenImageYet) {
      ...
      if (res.status == 200) {
        ...
        console.error('This media does not exist, or is no longer available on the server')
        haventGottenImageYet = false;
      } else {
        await timeout(2000);
      }
...
export const bleachMessage = (m : {
  ...
  Object.keys(m).map(key => {
    if (!["type", "clientUrl", "mimetype", "mediaKey", "size", "filehash"].includes(key)) delete r[key]
  })
  return r;
}